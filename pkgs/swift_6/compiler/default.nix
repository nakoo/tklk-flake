...
in stdenv.mkDerivation {
  ...
  patchPhase = ''
    ...
    patch -p1 -d swift_6 -i ${./patches/swift-wrap.patch}
    patch -p1 -d swift_6 -i ${./patches/swift-nix-resource-root.patch}
    patch -p1 -d swift_6 -i ${./patches/swift-linux-fix-libc-paths.patch}
    patch -p1 -d swift_6 -i ${./patches/swift-linux-fix-linking.patch}
    patch -p1 -d swift_6 -i ${./patches/swift-darwin-libcxx-flags.patch}
    patch -p1 -d swift_6 -i ${substituteAll {
    ...
    patch -p1 -d swift_6 -i $TMPDIR/swift-separate-lib.patch
    ...
    patch -p1 -d llvm-project/clang -i ${fetchpatch {
      name = "purity.patch";
      url = "https://raw.githubusercontent.com/NixOS/nixpkgs/201ef33028bbb02cbcb91eaaa6520157af71b412/pkgs/development/compilers/llvm/15/clang/purity.patch";
      hash = "sha256-gWnCA6ByB/t7B8FD/QXlYmgOT+OzsxJGe5MI+o295cs=";
    }}
    ...
    sed -i 's|''${GLIBC_INCLUDE_PATH}/uuid/uuid.h|${libuuid.dev}/include/uuid/uuid.h|' \
      swift_6/stdlib/public/Platform/glibc.modulemap.gyb
    ''}

    # LLDB Deprecated function has a conflict when using SWIG, so no-op them instead
    substituteInPlace llvm-project/lldb/include/lldb/lldb-defines.h \
      --replace-fail '#define LLDB_DEPRECATED_FIXME(MSG, FIX)' '#define LLDB_DEPRECATED_FIXME(MSG, FIX) // no-op ' \
      --replace-fail '#define LLDB_DEPRECATED(MSG)' '#define LLDB_DEPRECATED(MSG) // no-op' \
      --replace-fail '[[deprecated("This method is no longer supported: " MSG)]]' ""

    # Remove tests for cross compilation, which we don't currently support.
    rm swift_6/test/Interop/Cxx/class/constructors-copy-irgen-*.swift
    rm swift_6/test/Interop/Cxx/class/constructors-irgen-*.swift

    # TODO: consider fixing and re-adding. This test fails due to a non-standard "install_prefix".
    rm swift_6/validation-test/Python/build_swift.swift

    # We cannot handle the SDK location being in "Weird Location" due to Nix isolation.
    rm swift_6/test/DebugInfo/compiler-flags.swift

    # TODO: Fix issue with ld.gold invoked from script finding crtbeginS.o and crtendS.o.
    rm swift_6/test/IRGen/ELF-remove-autolink-section.swift
    ...
    rm swift_6/validation-test/BuildSystem/default_build_still_performs_epilogue_opts_after_split.test
    rm swift_6/validation-test/BuildSystem/test_early_swift_driver_and_infer.swift

    # TODO: This test fails for some unknown reason
    rm swift_6/test/Serialization/restrict-swiftmodule-to-revision.swift

    # This test was flaky in ofborg, see #186476
    rm swift_6/test/AutoDiff/compiler_crashers_fixed/issue-56649-missing-debug-scopes-in-pullback-trampoline.swift
  ...
  buildPhase = ''
    ...
    cmakeFlags="
      -GNinja
      -DLLDB_SWIFTC=$SWIFT_BUILD_ROOT/swift_6/bin/swiftc
      -DLLDB_SWIFT_LIBS=$SWIFT_BUILD_ROOT/swift_6/lib/swift
      -DLLVM_DIR=$SWIFT_BUILD_ROOT/llvm/lib/cmake/llvm
      -DClang_DIR=$SWIFT_BUILD_ROOT/llvm/lib/cmake/clang
      -DSwift_DIR=$SWIFT_BUILD_ROOT/swift_6/lib/cmake/swift
  ...
  installPhase = ''
    ...
    mv swift_6/bin/swift-frontend{-unwrapped,}
  ...
  meta = {
    ...
    # NOTE: Only target darwin for this flake, in nixpkgs it should be more
